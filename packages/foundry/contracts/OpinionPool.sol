...
contract OpinionPool is OwnableUpgradeable {
    ...
    event SharesBought(address indexed user, uint256 indexed optionId, uint256 amount, uint256 cost);
    event SharesSold(address indexed user, uint256 indexed optionId, uint256 amount, uint256 profit);

    // Initialization function (replaces constructor)
    function initialize(address _token, string memory _name, string[] memory _optionNames) public initializer {
    ...
    function _quotePrice(uint256 _optionId, uint256 _amount, bool _isBuy) internal view returns (uint256) {
    ...
    function quote(uint256 _optionId, uint256 _amount, bool _isBuy) public view returns (uint256) {
        require(_optionId < totalOptions, "Invalid option");
